    ...
    lib.mkMerge [
      {
        services = {
          blocky = {
            ...
            settings = {
              ports = {
                dns = builtins.map ({ addr, port }: "${addr}:${builtins.toString port}") cfg.listenAddresses;
              ...
              customDNS = {
                ...
                mapping = builtins.mapAttrs (
                  _: { ipv4, ipv6 }: "${infra.ipAddress ipv4},${infra.ipAddress ipv6}"
                ) cfg.localDomains;
              };
            };
          };
        ...
        systemd.services.blocky.serviceConfig.ExecStart =
          let
            ...
            configFile = format.generate "config.yaml" config.services.blocky.settings;
          in
          lib.mkForce "${pkgs.unstable.blocky}/bin/blocky --config ${configFile}";
      }
      ...
      (lib.mkIf config.services.postgresql.enable {
        age.secrets = {
          blocky-grafana-postgresql-grafana = {
            file = "${secrets}/secrets/other/whitelodge/blocky-grafana-postgresql.age";
            mode = "0640";
            owner = "root";
            group = "grafana";
          };
            ...
            provision.datasources.settings.datasources = [
              {
                ...
                secureJsonData.password = "$__file{${config.age.secrets.blocky-grafana-postgresql-grafana.path}}";
              }
            ];
          };
        };
